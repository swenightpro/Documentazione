...
% CORREZIONE: UC_11 e UC_12 sono UC indipendenti senza figura condivisa.
% Il diagramma UC_11 mostra: Utente --> UC_11
% Il diagramma UC_12 mostra: Utente --> UC_12 (standalone, nessun legame con UC_11)
...
\begin{figure}[H]
    ...
    \caption{UC\_11: Ricerca Prodotti a Catalogo}
\end{figure}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Il sistema intercetta l'input e interroga dinamicamente il database locale.
        \item Il sistema genera e aggiorna a schermo la lista contestuale dei risultati e, per ogni referenza, espone la scheda prodotto standard.
    \end{enumerate}
...
\begin{figure}[H]
    ...
    \caption{UC\_12: Configurazione e Aggiunta Prodotto al Carrello}
\end{figure}

\begin{itemize}
    \item \textbf{Attore Principale}: Utente
    \item \textbf{User Story}: Come Utente, dopo aver individuato un articolo di interesse, voglio poterne specificare la quantità desiderata e confermarne l'inserimento nel carrello corrente.
    \item \textbf{Scenario Principale}: 
    \begin{enumerate}
        \item L'Utente seleziona un prodotto dall'interfaccia prodotti.
    ...
    \end{itemize}
    \item \textbf{Trigger}: Selezione esplicita di una referenza dall'interfaccia prodotti.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Specializzazioni}:
    \begin{itemize}
        \item \hyperref[uc_14.1]{UC\_14.1}, \hyperref[uc_14.2]{UC\_14.2}
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Attori}: Utente (Attore Primario), Sistema ERP (Attore Secondario)
    \item \textbf{User Story}: Come Utente (Cliente nel front-end o Operatore in subentro durante un ticket), ritenendomi soddisfatto della lista creata, voglio richiedere la sottomissione formale della richiesta affinché venga processata a livello commerciale e logistico.
    \item \textbf{Scenario Principale}: 
    \begin{enumerate}
        \item L'Utente richiede formalmente la finalizzazione attivando il controllo primario di checkout. Nel caso dell'Operatore, l'azione viene eseguita dalla console di subentro (\hyperref[uc_22]{UC\_22}) per conto del Cliente assistito.
    ...
    \end{enumerate}
    \item \textbf{Precondizioni}: La lista articoli conta almeno una referenza valida a database. Per il Cliente, non vi sono ticket di assistenza pendenti o blocchi attivi. Per l'Operatore, deve esistere un ticket in stato ``In lavorazione'' con Lock attivo sulla sessione del Cliente.
    \item \textbf{Postcondizioni}: I dati di carrello escono dal perimetro della webapp. La sessione viene ripulita e l'Utente viene instradato alla vista appropriata al proprio ruolo: il Cliente alla dashboard dello Storico Ordini; l'Operatore alla chiusura automatica del ticket e alla dismissione della console di subentro.
    \item \textbf{Estensioni}:
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Trigger}: Azione intenzionale dell'Utente sul pulsante di completamento ordine.
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{UC\_19: Apertura Ticket Assistenza}
\end{figure}
...
\begin{itemize}
    ...
    \begin{enumerate}
        ...
        \item Il sistema espone lo storico della conversazione per contestualizzare la problematica $\rightarrow$ Include: \hyperref[uc_07]{UC\_07}
        \item Il sistema espone lo stato attuale del carrello in delega $\rightarrow$ Include: \hyperref[uc_13]{UC\_13}
        \item Contestualmente al carrello, esposizione del Livello di Confidenza AI sulle referenze elaborate automaticamente $\rightarrow$ Include: \hyperref[uc_22.1]{UC\_22.1}
    \end{enumerate}
    \item \textbf{Nota Operativa}: Una volta concluso questo scenario principale (Presa in carico ed esposizione del contesto), l'Operatore ha ottenuto le chiavi d'accesso per operare nell'ambiente del Cliente, potendo quindi avvalersi dei normali casi d'uso indipendenti del sistema (es. \hyperref[uc_08.1]{UC\_08.1} per chattare, \hyperref[uc_11]{UC\_11}..\hyperref[uc_14]{UC\_14} per modificare il carrello, \hyperref[uc_15]{UC\_15} per chiudere l'ordine).
    \item \textbf{Precondizioni}:
    \begin{itemize}
    ...
    \item \textbf{Postcondizioni}:
    \begin{itemize}
        \item L'Operatore assume e mantiene il pieno controllo asincrono (Lock) sui dati di sessione del Cliente.
    \end{itemize}
    \item \textbf{Inclusioni}:
    \begin{itemize}
        \item \hyperref[uc_00.2]{UC\_00.2}, \hyperref[uc_07]{UC\_07}, \hyperref[uc_13]{UC\_13}, \hyperref[uc_22.1]{UC\_22.1}
    \end{itemize}
    \item \textbf{Estensioni}:
    \begin{itemize}
...
\begin{figure}[H]
    \centering 
    \includegraphics[width=0.7\textwidth]{uc-27.png}
    \caption{UC\_27: Diagramma dei Casi d'Uso: Generalizzazione della Validazione}
\end{figure}
...
\begin{itemize}
    ...
    \item \textbf{Specializzazioni}:
    \begin{itemize}
        \item \hyperref[uc_27.1]{UC\_27.1}, \hyperref[uc_27.2]{UC\_27.2}
    \end{itemize}
...
\end{itemize}
    ...
    % ==========================================
    % APPENDICE: FUNZIONALITA' TRASVERSALI TABELLE OPERATIVE
    % ==========================================

    \subsection{UC\_29: Ordinamento Dinamico Tabelle Operative}
    \label{uc_29}

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.65\textwidth]{uc-29.png}
        \caption{UC\_29: Ordinamento Dinamico Tabelle Operative}
    \end{figure}

    \begin{itemize}
        \item \textbf{Attore Principale}: Operatore
        \item \textbf{User Story}: Come Operatore, voglio poter invertire l'ordinamento delle tabelle operative (Ordini, Ticket, Ottimizzazione) cliccando sull'intestazione di una colonna.
        \item \textbf{Scenario Principale}:
        \begin{enumerate}
            \item L'Operatore identifica la tabella corrente (storico ordini, ticket aperti o dashboard di ottimizzazione AI) e seleziona l'intestazione della colonna da ordinare.
            \item Il sistema calcola il nuovo ordinamento (ascendente/descendente), aggiorna l'indicatore di direzione e renderizza la tabella riordinata mantenendo eventuali filtri attivi.
        \end{enumerate}
        \item \textbf{Precondizioni}: Una delle tabelle operative (Ordini, Ticket, Ottimizzazione) è visualizzata e contiene almeno un record.
        \item \textbf{Postcondizioni}: I record della tabella corrente risultano riordinati secondo il criterio selezionato dall'Operatore.
        \item \textbf{Trigger}: Click dell'Operatore sull'intestazione di una colonna della tabella.
    \end{itemize}

    \subsection{UC\_30: Filtraggio Temporale Tabelle Operative}
    \label{uc_30}

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.65\textwidth]{uc-30.png}
        \caption{UC\_30: Filtraggio Temporale Tabelle Operative}
    \end{figure}

    \begin{itemize}
        \item \textbf{Attore Principale}: Operatore
        \item \textbf{User Story}: Come Operatore, voglio poter filtrare i record delle tabelle operative per intervallo temporale.
        \item \textbf{Scenario Principale}:
        \begin{enumerate}
            \item L'Operatore seleziona una data di inizio e una data di fine dal componente calendario associato alla tabella corrente (Ordini, Ticket, Ottimizzazione AI).
            \item Il sistema applica il filtro temporale sugli attributi data/ora disponibili (es. data ordine, data apertura ticket, timestamp log), ricalcola l'elenco e mostra solo i record nell'intervallo, preservando gli altri filtri in uso.
        \end{enumerate}
        \item \textbf{Precondizioni}: La tabella operativa visualizzata espone almeno un attributo data/ora filtrabile.
        \item \textbf{Postcondizioni}: La tabella mostra solo i record coerenti con il filtro temporale impostato.
        \item \textbf{Trigger}: Selezione di un intervallo di date nel filtro calendario.
    \end{itemize}

    \subsection{UC\_31: Ricerca Mirata per Colonna Tabelle Operative}
    \label{uc_31}

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.65\textwidth]{uc-31.png}
        \caption{UC\_31: Ricerca Mirata per Colonna Tabelle Operative}
    \end{figure}

    \begin{itemize}
        \item \textbf{Attore Principale}: Operatore
        \item \textbf{User Story}: Come Operatore, voglio poter cercare del testo all'interno di una colonna specifica delle tabelle operative (Ordini, Ticket, Ottimizzazione) per isolare rapidamente i record di interesse.
        \item \textbf{Scenario Principale}:
        \begin{enumerate}
            \item L'Operatore seleziona la colonna su cui ricercare (es. ID Cliente, ID Ordine, ID Ticket, Ragione Sociale, stato validazione log).
            \item L'Operatore digita il testo nel campo di ricerca dedicato.
            \item Il sistema filtra istantaneamente la vista mostrando solo i record corrispondenti nella colonna selezionata, mantenendo ordinamenti e filtri temporali già attivi.
        \end{enumerate}
        \item \textbf{Precondizioni}: Una delle tabelle operative è visualizzata.
        \item \textbf{Postcondizioni}: La tabella mostra solo i record corrispondenti alla ricerca nella colonna selezionata.
        \item \textbf{Trigger}: Immissione di testo nel campo di ricerca per colonna.
    \end{itemize}