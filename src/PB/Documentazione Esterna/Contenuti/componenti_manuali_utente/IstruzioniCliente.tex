...
\subsection{Input tramite immagini}

Per creare un ordine caricando un'immagine:

\begin{enumerate}
    \item \textbf{Attivazione}: Nella barra di input della chat, cliccare sull'icona della fotocamera o trascinare un'immagine direttamente nel pannello chat.
    \item \textbf{Caricamento}: Selezionare un'immagine dal dispositivo. Il sistema accetta formati comuni (JPEG, PNG, ecc.).
    \item \textbf{Analisi}: L'IA analizza l'immagine automaticamente per identificare i prodotti visibili (ad esempio, etichette, confezioni, ricevute) o riconosce il testo presente nell'immagine, incluso il testo manuale scritto a mano.
    \item \textbf{Risposta dell'IA}: L'IA risponde confermando i prodotti riconosciuti e le relative quantit\`a. Accanto a ciascun articolo comparir\`a un anello colorato che indica la confidenza della classificazione.
    \item \textbf{Errore}: Se l'IA non riesce a identificare i prodotti nell'immagine, risponde chiedendo di provare con un'immagine pi\`u chiara o di utilizzare la chat testuale.
\end{enumerate}
