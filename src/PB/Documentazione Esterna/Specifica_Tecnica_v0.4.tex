...
\subsubsection{Utilizzo del pattern nel progetto}
Applicato nel frontend Next.js per la gestione dello stato globale al Livello 2. I Context Provider (es. \texttt{AuthContext}, \texttt{CartContext}, \texttt{ChatContext}, \texttt{NotificationContext}) mantengono lo stato condiviso; i componenti UI che consumano tali contesti tramite hook fungono da Observer. Nel Livello 4, gli event handler realtime (\texttt{ChatEventHandler}, \texttt{TicketEventHandler}, \texttt{OrderEventHandler}, \texttt{AILogEventHandler}) ricevono eventi dal \textit{RT Gateway} e propagano aggiornamenti ai contesti, innescando il re-render selettivo delle viste interessate.
...
Questa sezione riporta lo stato di implementazione dei requisiti funzionali derivati dall'ultima versione PB dell'Analisi dei Requisiti (v2.1). Per questa baseline, tutti i requisiti risultano \textbf{NON IMPLEMENTATO}.

\renewcommand{\arraystretch}{1.15}
\begin{longtable}{|>{\raggedright\arraybackslash}p{0.15\textwidth}|>{\raggedright\arraybackslash}p{0.16\textwidth}|>{\raggedright\arraybackslash}p{0.45\textwidth}|>{\raggedright\arraybackslash}p{0.16\textwidth}|}
    \hline
    \rowcolor[gray]{0.9}
    ...
    \hline
    \endhead
    \texttt{RF-OB\_01} & Obbligatorio & Il sistema deve permettere all'Utente di inserire il proprio indirizzo email nel campo dedicato per effettuare il login. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_02} & Obbligatorio & Il sistema deve permettere all'Utente di inserire la propria password in modo oscurato tramite il campo dedicato. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_03} & Obbligatorio & Il sistema deve fornire un pulsante o comando per confermare l'accesso e inviare le credenziali per la validazione. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_04} & Obbligatorio & Il sistema deve mostrare un errore se l'email inserita non ha un formato valido. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_05} & Obbligatorio & Il sistema deve mostrare un errore se l'email inserita non e presente nel database locale. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_06} & Obbligatorio & Il sistema deve mostrare un errore se la combinazione di email e password e sbagliata. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_07} & Obbligatorio & Dopo un login valido, il sistema deve creare la sessione e far accedere l'utente alla propria area (Cliente o Operatore). & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_08} & Obbligatorio & Il sistema deve fornire un pulsante per effettuare il logout, invalidando e chiudendo la sessione corrente lato server. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_09} & Obbligatorio & Il sistema deve mostrare i dati aziendali del Cliente (ID e Ragione Sociale) in sola lettura. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_10} & Obbligatorio & Il sistema deve rendere accessibile la pagina Area Personale tramite un elemento di navigazione dedicato, consentendo all'Utente di consultare le informazioni del proprio profilo in base al ruolo assegnato. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_11} & Obbligatorio & Il sistema deve mostrare all'Utente il proprio indirizzo email in sola lettura. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_13} & Obbligatorio & Il sistema deve mostrare in tempo reale tutti i messaggi inviati dall'Utente nella chat corrente. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_14} & Obbligatorio & Il sistema deve mostrare in tempo reale le risposte testuali scritte dall'AI o dall'Operatore di supporto. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_15} & Obbligatorio & Il sistema deve fornire un campo di testo per l'inserimento e l'invio di messaggi testuali nella chat. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_17} & Obbligatorio & Il sistema deve permettere all'Utente la registrazione e l'invio di messaggi vocali tramite il microfono del dispositivo. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_18} & Obbligatorio & Il sistema deve convertire l'audio in testo usando un modulo Speech-to-Text. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_21} & Obbligatorio & Il Cliente deve poter resettare la chat con l'AI e svuotare il carrello attuale. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_23} & Obbligatorio & L'AI (LLM) deve capire quali prodotti vuole l'utente e in che quantita leggendo/ascoltando il messaggio. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_24} & Obbligatorio & Il sistema deve cercare i prodotti nel database usando la ricerca vettoriale (Vector Search). & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_25} & Obbligatorio & Il sistema deve aggiungere in automatico i prodotti trovati al carrello se il Confidence Score calcolato dall'AI supera la soglia definita in \hyperlink{rp-ob_3}{RP-OB\_03}. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_26} & Obbligatorio & Vincolo Assortimento: Il sistema deve ignorare i prodotti che non fanno parte del catalogo assegnato a quel Cliente (tabella \texttt{asscli}). & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_28} & Obbligatorio & Deduzione Formati: L'AI deve convertire automaticamente le quantita richieste nelle giuste unita di vendita (es. da bottiglie a casse). & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_29} & Obbligatorio & Il sistema deve fornire una barra di ricerca per cercare manualmente i prodotti nel catalogo tramite input testuale. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_30} & Obbligatorio & Il sistema deve mostrare i risultati della ricerca in tempo reale (Live Search). & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_31} & Obbligatorio & Il sistema deve mostrare per ogni prodotto trovato: Nome, ID, Linea, Famiglia e Modalita di Vendita. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_32} & Obbligatorio & L'Utente deve poter cliccare su un risultato per decidere quanti pezzi acquistarne. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_33} & Obbligatorio & L'Utente deve poter modificare la quantita usando i tasti +/- o scrivendo direttamente il numero. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_35} & Obbligatorio & L'Utente deve poter confermare l'aggiunta del prodotto al carrello della sessione corrente. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_36} & Obbligatorio & Il sistema deve mostrare un avviso di ``Carrello Vuoto'' se non ci sono prodotti inseriti. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_37} & Obbligatorio & Il sistema deve mostrare i dettagli logistici e le quantita di tutti i prodotti presenti nel carrello. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_38} & Obbligatorio & L'Utente deve poter sbloccare la riga di un prodotto nel carrello per poterne modificare la quantita. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_39} & Obbligatorio & L'Utente deve poter salvare la nuova quantita modificata per un prodotto nel carrello. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_40} & Obbligatorio & L'Utente deve poter rimuovere definitivamente un articolo dal carrello. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_41} & Obbligatorio & Il sistema deve disabilitare il pulsante per inviare l'ordine se il carrello e vuoto. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_42} & Obbligatorio & L'Utente (Cliente o Operatore in subentro) deve poter chiudere il carrello e inviare l'ordine definitivo. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_44} & Obbligatorio & Dopo l'invio dell'ordine, il sistema deve svuotare la sessione e reindirizzare l'Utente alla vista appropriata al proprio ruolo: il Cliente allo Storico Ordini, l'Operatore alla chiusura del ticket e dismissione della console di subentro. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_45} & Obbligatorio & Il sistema deve creare un file JSON formattato contenente tutti i dati dell'ordine appena chiuso. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_46} & Obbligatorio & Il Cliente deve poter visualizzare la lista dei propri ordini passati, ordinati per data. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_47} & Obbligatorio & Il sistema deve mostrare per ogni ordine storico: ID, Data, totale degli articoli e una piccola anteprima dei prodotti. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_48} & Obbligatorio & Il Cliente deve poter cliccare su un ordine passato per vederne tutti i dettagli logistici. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_49} & Obbligatorio & Il sistema deve mostrare in sola lettura la chat originale tra utente e AI che ha generato quell'ordine specifico. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_50} & Obbligatorio & Il Cliente deve poter richiedere l'aiuto di un operatore umano, mettendo in pausa l'AI. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_51} & Obbligatorio & L'Operatore deve poter accedere alla dashboard con la lista dei Ticket Aperti (richieste d'aiuto). & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_52} & Obbligatorio & Nella lista dei ticket, il sistema deve mostrare da quanto tempo il Cliente sta aspettando una risposta. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_53} & Obbligatorio & L'Operatore deve poter prendere in carico un ticket bloccandolo per gli altri colleghi (Lock) per aprire la console di assistenza. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_54} & Obbligatorio & L'Operatore deve poter scrivere e inviare messaggi in chat direttamente al Cliente per fornirgli assistenza. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_55} & Obbligatorio & L'Operatore deve poter usare la ricerca per aggiungere forzatamente prodotti al carrello del Cliente che sta aiutando. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_56} & Obbligatorio & L'Operatore deve poter modificare le quantita o rimuovere prodotti dal carrello del Cliente. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_57} & Obbligatorio & L'Operatore deve poter chiudere il ticket per segnare il problema come risolto e sbloccare la sessione (rimuovere il Lock). & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_58} & Obbligatorio & L'Utente (Cliente o Operatore) deve poter chiudere il ticket da solo se risolve il problema o non ha piu bisogno di aiuto. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_59} & Obbligatorio & L'Operatore deve poter accedere alla dashboard che mostra tutti gli ordini inviati da tutti i Clienti. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_60} & Obbligatorio & L'Operatore deve poter cliccare su un ordine di qualsiasi cliente per aprirlo e vederne i dettagli. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_61} & Obbligatorio & L'Operatore deve poter cercare del testo solo all'interno di una colonna specifica (es. cercare solo l'ID Ordine). & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_62} & Obbligatorio & L'Operatore deve poter ordinare le tabelle in modo crescente o decrescente cliccando sui nomi delle colonne. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_63} & Obbligatorio & L'Operatore deve poter filtrare i dati delle tabelle scegliendo una data di inizio e di fine. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_64} & Obbligatorio & Il sistema deve salvare nel database tutto lo storico delle chat e delle decisioni prese dall'AI. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_65} & Obbligatorio & Il sistema deve mostrare la percentuale di sicurezza dell'AI (Confidence Score) solo nelle schermate dedicate all'Operatore. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_66} & Obbligatorio & Il sistema deve mostrare schermate di caricamento (spinner o barre di avanzamento) durante le operazioni che richiedono un tempo di elaborazione superiore a 1 secondo. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_67} & Obbligatorio & Le API del server (Backend) devono rifiutare qualsiasi richiesta priva di un token di accesso valido per la sessione corrente. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-DE\_03} & Desiderabile & Il sistema deve permettere all'Utente loggato di avviare la procedura per cambiare la propria password. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-DE\_04} & Desiderabile & L'Utente deve poter inviare la vecchia password e la nuova password scelta per salvarla nel sistema. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-DE\_05} & Desiderabile & Il sistema deve bloccare il cambio password se la vecchia password e errata o se le due nuove password inserite non coincidono. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-DE\_08} & Desiderabile & Il Cliente deve poter valutare la qualita delle risposte dell'AI lasciando un feedback (es. pollice su / pollice giu). & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-DE\_09} & Desiderabile & Se il Cliente da un voto negativo all'AI, il sistema deve richiedere che indichi il tipo di errore (tramite tag predisposti). & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-DE\_10} & Desiderabile & Il Cliente deve poter aggiungere un testo libero per spiegare meglio il motivo dell'errore segnalato. & \textbf{NON IMPLEMENTATO} \\
    \hline
