...
Questa sezione riporta lo stato di implementazione dei requisiti funzionali derivati dall'ultima versione PB dell'Analisi dei Requisiti (v2.1). Per questa baseline, lo stato di ciascun requisito è indicato nella colonna \textbf{Stato}.
...
\begin{longtable}{|>{\raggedright\arraybackslash}p{0.15\textwidth}|>{\raggedright\arraybackslash}p{0.16\textwidth}|>{\raggedright\arraybackslash}p{0.45\textwidth}|>{\raggedright\arraybackslash}p{0.25\textwidth}|}
    ...
    \hline
    \texttt{RF-OB\_12} & Obbligatorio & Il sistema deve esporre un comando di contatto che, alla sua attivazione, invochi il protocollo URI \textit{mailto:} per aprire il client di posta elettronica nativo dell'Utente con il campo destinatario pre-compilato con l'email di supporto aziendale. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_16} & Obbligatorio & Il sistema deve bloccare l'invio e mostrare un errore se il messaggio di testo supera il limite di caratteri. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_19} & Obbligatorio & Il sistema deve bloccare l'invio e mostrare un errore se il file audio pesa piu di 10MB. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-OB\_20} & Obbligatorio & Il sistema deve richiedere esplicitamente i permessi hardware necessari (microfono, fotocamera) al momento del primo utilizzo della relativa funzionalita e, in caso di rifiuto da parte dell'Utente, mostrare un messaggio informativo che spieghi l'impossibilita di procedere senza tali permessi. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_22} & Obbligatorio & Il sistema deve disabilitare il comando di reset della sessione qualora sia presente un ticket di assistenza in stato ``Aperto'' o ``In lavorazione'' per la sessione corrente, impedendo all'Utente di azzerare il contesto durante un'assistenza attiva. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_27} & Obbligatorio & Vincolo Stato: Il sistema deve impedire l'aggiunta di prodotti segnati come ``non disponibili'' o ``sospesi'' nel database (\texttt{anaart}). & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_34} & Obbligatorio & Il sistema deve correggere in automatico la quantita se l'utente inserisce numeri negativi, zero o troppo grandi. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_43} & Obbligatorio & Il sistema deve bloccare l'invio dell'ordine e segnalare gli errori se ci sono problemi con le regole del gestionale (es. prodotto fuori catalogo). & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OB\_68} & Obbligatorio & Il sistema deve caricare i dati delle tabelle in modo incrementale tramite paginazione con pagine da massimo 50 record, per evitare il blocco dell'interfaccia in presenza di grandi volumi di dati. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-DE\_06} & Desiderabile & Il sistema deve permettere l'invio di file immagine nella chat, mediante scatto fotografico o selezione dalla galleria del dispositivo. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \texttt{RF-DE\_07} & Desiderabile & Il sistema deve bloccare l'invio di immagini e mostrare un errore se il file pesa piu di 5MB. & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-DE\_12} & Desiderabile & L'Operatore deve poter accedere alla dashboard per il controllo degli errori e l'addestramento dell'AI (Ottimizzazione AI). & \textbf{NON IMPLEMENTATO} \\
    \hline
    ...
    \hline
    \texttt{RF-OP\_03} & Facoltativo & Il sistema deve generare grafici e statistiche generali sull'uso della piattaforma, mostrandoli all'Operatore. & \textbf{NON IMPLEMENTATO} \\
    \hline
    \multicolumn{4}{c}{} \\
    \caption{Stato dei requisiti funzionali (fonte: Analisi dei Requisiti PB v2.1)}\label{tab:stato_requisiti_funzionali}
\end{longtable}
