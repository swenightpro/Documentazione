...
\subsubsection{Descrizione} Questa sezione definisce le norme, gli strumenti e le metodologie che ogni membro del gruppo \textit{NightPRO} si impegna a rispettare. L'obiettivo è garantire una collaborazione efficace e trasparente con l'azienda proponente, \textit{Ergon Informatica}.
...
\subsubsection{Gestione della Comunicazione} Il gruppo \textit{NightPRO} si impegna a garantire una comunicazione costante e trasparente con \textit{Ergon Informatica} per tutta la durata del progetto, interfacciandosi primariamente con il referente Gianluca Carlesso. Gli incontri periodici con il referente sono finalizzati a: \begin{itemize} 
    \item Discutere i requisiti (necessari, desiderabili, facoltativi) da includere nel prodotto. 
    \item Ricevere feedback sullo stato di avanzamento dei lavori. 
...
\begin{itemize}
    \item \textbf{Progettazione Dettagliata (Design)}: Definizione dell'architettura software, delle interfacce dei moduli e dei pattern da implementare, partendo dai requisiti analizzati.
...
\subsubsection{Analisi dei Requisiti}
    \textbf{\textit{Placeholder (Da definire)}}
...
struttura del sistema, le specifiche funzionali e non funzionali, 
...
\begin{itemize}
    \item \textbf{Progettazione logica}: Questa fase si concentra sulla selezione dello stack tecnologico (tecnologie, \textit{framework}, librerie). L'obiettivo è motivare l'adeguatezza di tali scelte e confermarne la fattibilità tecnica, tipicamente attraverso la realizzazione di un \textit{Proof of Concept} (PoC).
...
\item \textbf{Prodotto software}: attualmente vuoto, verrà popolato quando inizierà la fase di codifica software.
\end{itemize}
...
\paragraph{Telegram} Per le comunicazioni operative e le richieste che necessitano di una risposta rapida da e verso l'azienda, si utilizza l'applicazione di messaggistica istantanea Telegram. Questo canale è dedicato alla gestione agile delle questioni quotidiane e agli aggiornamenti tempestivi che non richiedono la struttura formale di un'email.
